...
struct TypeExist<T, H, R...>
    : std::conditional<std::is_same<T, H>::value, std::true_type,
                       TypeExist<T, R...>>::type {};
...
class Variant {
 public:
  ...
  template <typename V,
            typename = typename std::enable_if<TypeExist<
                typename std::remove_reference<V>::type, T...>::value>::type>
  ...
  ~Variant() { VariantHelper<T...>::Destory(mem_type_id_, &data_); }
  ...
  bool Is() const {
    return mem_type_id_ ==
           std::type_index(typeid(typename std::remove_reference<V>::type));
  }

  template <typename V>
  V &Get() {
    if (Is<V>())
      return *reinterpret_cast<typename std::remove_reference<V>::type *>(
          &data_);
    else
      throw std::bad_cast();
  }
  ...
  bool IsValid() const { return mem_type_id_ != std::type_index(typeid(void)); }
