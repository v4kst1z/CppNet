...
#ifndef CPPNET_THREADPOOLBASE_H
#define CPPNET_THREADPOOLBASE_H
...
template<typename Func, typename ...Args>
inline auto ThreadPool::enqueue(Func&& f, Args && ...args) -> std::future<decltype(std::forward<Func>(f)(std::forward<Args>(args)...))> {
...
uint8_t ThreadPool::GetThreadNum() {
    return num_threads_;
}
